...
- Tiered models (DR-9, supersedes DR-3): both tiers are Anthropic on one key — cheap `claude-haiku-4-5` reads every document, strong `claude-sonnet-5` is asked as well when confidence < threshold or a critical flag is present. Thresholds configurable. Each tier is its own client built from `baymax.extract.vision-model` / `.strong-model`, independent of MedScribe's `llm.provider`; before DR-9 the strong tier was "the default client when `llm.provider=anthropic`", which meant escalation silently never fired while the default was Groq. **Groq is dropped from the Baymax pipeline**: its free tier caps output at 1,000 tokens/minute against the ~1,810 one document declares, so every call 429s before the model runs — one document per two minutes across all families, and no pacing fixes a per-minute budget. The Groq client stays in `elioo-llm` and in config, unused by Baymax, and returns as the cheap tier if the account is ever upgraded.
